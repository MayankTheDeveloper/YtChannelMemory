...
def long_form_recommendation_node(state: ChannelAnalysisState) -> ChannelAnalysisState:
    ...
    combined_insights = {
        ...
        "long_form_insights": state.get("long_form_insights"),
        "competitor_insights": state.get("competitor_insights"),
        "trend_insights": state.get("trend_insights"),
        "long_form_memory_context": state.get("long_form_memory_context")
    }
    
    system_prompt = """
    You are an expert YouTube Long-Form content strategist powered by Hindsight Memory. 
    Based on the provided Long-Form insights and the 'long_form_memory_context', generate exactly 10 highly confident Long-Form video ideas optimized for watch time, subscriber growth, and revenue.
    ...
    try:
        ...
        state["long_form_recommendations"] = recommendations
    except Exception as e:
        print(f"Error generating Long Form recommendations: {e}")
        state["long_form_recommendations"] = []
